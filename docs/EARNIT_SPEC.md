...
**Summary:** 220 unit tests across 28 test files. 120 instrumented tests across 32 files (requires device/emulator) — including 87 Compose UI tests.
...
│   ├── Task Edit            — name, icon, group, manual or auto points (sliders); group field is a collapsible section (expanded by default, collapses to "Group · [name]" summary) with a filled-card radio-button list of existing groups, matching the styling of the auto-points and "Use to get:" sections; "New group..." is the last row in the list — a radio + inline text field; selecting a radio clears the new-group field and vice versa; duplicate name blocked with inline error on the name field (suppressed briefly while save navigation is in flight); saving a new task navigates to TaskDetailScreen; saving an existing task shows "Task saved" snackbar and stays on screen; when opened from a new-reward form shows "Will be added to: [reward name]" below the name field and pops back to the reward form on save instead of forward to TaskDetailScreen
