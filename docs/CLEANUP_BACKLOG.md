...
| File | Size | Status |
|---|---|---|
| `RewardDetailScreen.kt` | was 658 lines, one composable | **Done** on `refactor/split-reward-detail-screen` (uncommitted) — split into a ~130-line orchestrator plus 6 private helpers (`RewardDetailDialogs`, `RewardHeaderCard`, `RewardProgressBar`, `RewardClaimOrLogButton`, `RewardTasksSection`, `RewardActivitySection`); ktlint/build/unit tests pass; manual pass in progress |
| `TaskEditScreen.kt` | ~637 lines in `TaskEditScreen(...)` before `EmojiPickerDialog`/`SliderRow` helpers | Planned — `refactor/split-task-edit-screen` |
| `SettingsScreen.kt` | ~430 lines in `SettingsScreen(...)` before the first helper | Planned — `refactor/split-settings-screen` |
