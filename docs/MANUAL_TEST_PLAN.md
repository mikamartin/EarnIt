...
5. Confirm the widget briefly shows "✓ Logged!" (flash), then reverts to normal state.
...
8. Configure the widget with a reward that has tasks. Log enough points to meet the point cost **without** completing all mandatory (★) tasks. Spot-check the exclamation icon (!) appears inline next to the reward name, immediately after the visible name text (not pushed to the far edge of the row), on a real render — the hint-vs-no-hint branch itself is covered by `standardContent_mandatoryTaskUnloggedButPointsMet_showsHint` / `standardContent_notBlockedOnMandatoryTask_hidesHint`.
9. Log tasks until all non-repeatable tasks for the tracked reward are done, with points still below the point cost (tasks default to repeatable now, so toggle any test task to non-repeatable when linking it to the reward to set this state up). Spot-check the disabled **+ LOG** button (muted, not tappable) and the same inline exclamation icon both render correctly on a real widget host (covered by `standardContent_allTasksDoneBelowCost_showsDisabledLogButtonAndHint`).
...
14. With a reward in the mandatory-task-hint state from step 8, drag-resize the widget down to its smallest footprint (e.g. a 3-column-wide, 2-row-tall home screen grid cell). Confirm the reward name, exclamation icon, and full progress bar all stay visible with no clipping — this combination (narrow width + minimal height) is what caused the progress bar to be cut off on at least one device, first via `fix/widget-hint-overflow` (hint text wrapping to 2 lines), then again via `fix/widget-progress-bar-clipping` (the hint's second line alone was enough extra height to clip the bar even at one line — closed by moving the hint to a same-line icon that adds no height). Repeat at the same footprint with a reward in the all-tasks-done state from step 9 — same icon-plus-disabled-button layout, same clipping risk if a future change reintroduces a second line here.
