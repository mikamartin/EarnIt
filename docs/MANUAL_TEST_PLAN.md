...
8. Configure the widget with a reward that has tasks. Log enough points to meet the point cost **without** completing all mandatory (★) tasks. Spot-check the info icon (ⓘ) appears inline next to the reward name on a real render — the hint-vs-no-hint branch itself is covered by `standardContent_mandatoryTaskUnloggedButPointsMet_showsHint` / `standardContent_notBlockedOnMandatoryTask_hidesHint`.
9. Log tasks until all non-repeatable tasks for the tracked reward are done, with points still below the point cost (tasks default to repeatable now, so toggle any test task to non-repeatable when linking it to the reward to set this state up). Spot-check the disabled **+ LOG** button (muted, not tappable) and the same inline info icon both render correctly on a real widget host (covered by `standardContent_allTasksDoneBelowCost_showsDisabledLogButtonAndHint`).
