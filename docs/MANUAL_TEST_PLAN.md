...
9. Log tasks until all non-repeatable tasks for the tracked reward are done, with points still below the point cost (tasks default to repeatable now, so toggle any test task to non-repeatable when linking it to the reward to set this state up). Spot-check the disabled **+ LOG** button (muted, not tappable) and the "All tasks done — add more" hint subtitle both render correctly on a real widget host (covered by `standardContent_allTasksDoneBelowCost_showsDisabledLogButtonAndHint`).
