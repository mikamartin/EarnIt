...
- [ ] Does any new instrumented test class carry its required layer tag (`@RepositoryTest`/`@UtilityTest`/`@UiTest`) and at least one optional tag (see TESTING.md's Tagging convention)? `./gradlew check` runs `checkInstrumentedTestTags` and fails if either is missing.
