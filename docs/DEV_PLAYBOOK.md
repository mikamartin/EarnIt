...
- `StandardContent` in `EarnItWidget.kt` has no general overflow protection: content is a fixed-height `Column` centered inside a fixed-size `Box` (`fea294c`, kept to avoid dead space on tall launcher grid cells), so if rendered content height ever exceeds the actual widget box, `Alignment.Center` crops evenly off the top and bottom with no scroll or shrink fallback. This has bitten the mandatory-task/all-tasks-done hint twice from two different triggers: first the hint text wrapping to 2 lines on narrow widths (bounded with `maxLines = 1` in `fix/widget-hint-overflow`), then the hint's second line itself being enough extra height to clip the progress bar on short/resized widgets even at one line (closed in `fix/widget-progress-bar-clipping` by replacing the text line with a static icon inline next to the reward name, carrying the explanation via `contentDescription` instead of visible text — so the hint state no longer adds height at all). Any other future source of extra height — a longer reward name at large accessibility font scale, a new line added to the layout — would still reproduce the same clipping. Revisit with a proper responsive layout (e.g. `LocalSize`-driven, matching the pattern already used in `ProgressBar`) if this recurs.
- Play Console's pre-launch report flags `Window.setStatusBarColor`/`setNavigationBarColor`/`LAYOUT_IN_DISPLAY_CUTOUT_MODE_SHORT_EDGES` as deprecated APIs. These aren't called from app code — they're internal to `androidx.activity`'s `enableEdgeToEdge()` implementation, used as its fallback on API 31–34 (edge-to-edge only becomes OS-native from API 35, and `minSdk` is 31). Not actionable without dropping support for Android 12–14; expect this warning on every release until then.
