...
| `WidgetContentTest` (12) | Renders `StandardContent`/`FlashContent`/`EmptyState`/`ClaimedState` via `glance-testing` + Robolectric (JVM, no device) — correct button shown/hidden per state with click action wired, disabled non-clickable `LOG_DISABLED` button plus hint text when all one-time tasks are done, reward name/points/custom-label text, mandatory hint shown/hidden, flash and empty/claimed state text |
